...
```php
LKN_FILEBROWSER_VERSION       // '1.0.0'
LKN_FILEBROWSER_PLUGIN_NAME                // 'linknacional-file-browser'
LKN_FILEBROWSER_PLUGIN_URL    // plugin_dir_url(__FILE__)
LKN_FILEBROWSER_PLUGIN_PATH   // plugin_dir_path(__FILE__)
```
- Sempre usar `LKN_FILEBROWSER_PLUGIN_URL` e `LKN_FILEBROWSER_PLUGIN_PATH`. Nunca `plugin_dir_path( __FILE__ )` direto.
- `LKN_FILEBROWSER_PLUGIN_URL` + `LKN_FILEBROWSER_VERSION` como cache-buster em `wp_enqueue_style`/`wp_enqueue_script`.

### Prefixos
- Options/DB: `lkn_filebrowser_*` (ex: `lkn_filebrowser_db_version`)
- Hooks/AJAX actions: `lkn_*` (ex: `wp_ajax_lkn_create_folder`, `wp_ajax_lkn_frontend_*`)
- JS handles: `lkn-filebrowser` (admin e public — mesmo handle, hooks diferentes)
- JS globals: `lkn_ajax` (admin), `lkn_public_ajax` (public)
- Nonces: `lkn_filebrowser_nonce`, `lkn_filebrowser_admin_nonce`, `lkn_filebrowser_public_nonce`
- Shortcode: `[lkn_filebrowser]`
...
- `{prefix}lkn_filebrowser_folders` — id, name, parent_id, path, created_at, updated_at
- `{prefix}lkn_filebrowser_files` — id, name, original_name, folder_id, file_type, file_size, file_path, file_url, description, created_at, updated_at
...
- Arquivos físicos em `wp-content/uploads/lkn-filebrowser/` (criado via `wp_mkdir_p` na ativação).
...
|---|---|---|
| `assets/js/compiled/fontawesome.compiled.js` | Admin + Public (compartilhado) | `lkn-filebrowser-fontawesome` |
...
- Localize com `wp_localize_script()` — admin usa `lkn_ajax`, public usa `lkn_public_ajax`.
- **Qualquer string visível no JS deve vir via `wp_localize_script()`** com text domain. Nunca hardcoded no JS (nem em inglês). Fallback: `${lkn_ajax.key || 'Default English'}`.
...
- Admin enfileira **somente** na página do plugin (`toplevel_page_lkn-filebrowser`), via `$hook_suffix`.
- Public enfileira **somente** quando `[lkn_filebrowser]` é renderizado — `wp_enqueue_style`/`wp_enqueue_script` dentro do shortcode, não via `wp_enqueue_scripts`.
...
- Admin nonces: `lkn_filebrowser_nonce`, `lkn_filebrowser_admin_nonce`.
- Public nonces: `lkn_filebrowser_public_nonce` (obtido via AJAX `lkn_get_public_nonce`).
...
5. `plugin_dir_path( __FILE__ )` — usar constantes `LKN_FILEBROWSER_PLUGIN_PATH`/`LKN_FILEBROWSER_PLUGIN_URL`.
