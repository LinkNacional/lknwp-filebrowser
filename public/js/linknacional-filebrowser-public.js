(function ($) {
	...
	$(document).ready(function () {
		...
		fetchAndSetNonce('linknacional_filebrowser_public_nonce', function () {
			$('.linknacional-filebrowser-public').each(function () {
		...
		$('#linknacional-search-input').on('input', function () {
		...
		// Search button click
		$('#linknacional-search-btn').on('click', function () {
			const searchTerm = $('#linknacional-search-input').val().trim();
			if (searchTerm.length >= 2) {
				performSearch(searchTerm);
			}
		});
		...
		// Clear search button
		$('#linknacional-clear-search').on('click', function () {
			clearSearch();
		});

		// Enter key search
		$('#linknacional-search-input').on('keypress', function (e) {
			if (e.which === 13) {
				e.preventDefault();
				const searchTerm = $(this).val().trim();
				if (searchTerm.length >= 2) {
		...
		$('.layout-btn').on('click', function () {
			...
			const $container = $('.linknacional-filebrowser-public');
			const $content = $('.linknacional-filebrowser-content');
		...
		$(document).on('click', '.content-item.file, .content-item.search-result-file', function (e) {
			...
			const $container = $(this).closest('.linknacional-filebrowser-public');
			const $treeContainer = $container.find('#linknacional-folder-tree-public');
			...
			if (isSearchMode) {
				$('#linknacional-search-input').val('');
				$('#linknacional-clear-search').hide();
				isSearchMode = false;
			}
			...
			setTimeout(() => {
				...
				if ($toggleBtn.length > 0) {
					...
					$toggleBtn.attr('title', linknacional_public_ajax.hide_subfolders || 'Hide Subfolders');
				}
			}, 300);
		});
	...
	function fetchAndSetNonce(actionName, onReady) {
		$.ajax({
			url: linknacional_public_ajax.ajax_url,
			type: 'POST',
			data: { action: 'linknacional_get_public_nonce', action_name: actionName },
	...
	function loadFolderContents(folderId, $container, skipTreeUpdate = false, callback = null) {
		const $content = $container.find('.linknacional-filebrowser-content');

		$content.html('<div class="loading"><i class="fas fa-spinner"></i> ' + (linknacional_public_ajax.loading_text || 'Loading...') + '</div>');

		$.ajax({
			url: linknacional_public_ajax.ajax_url,
			type: 'POST',
			data: {
				action: 'linknacional_frontend_get_contents',
				nonce: nonce,
				folder_id: folderId
			},
			success: function (response) {
				if (response.success) {
					...
					$content.html('<div class="error">' + (linknacional_public_ajax.error_loading_text || 'Error loading contents') + '</div>');
				}
			},
			error: function () {
				$content.html('<div class="error">' + (linknacional_public_ajax.error_loading_text || 'Error loading contents') + '</div>');
		...
		if (folders.length === 0 && files.length === 0) {
			$content.html(`
				<div class="empty-folder">
					...
					<h3>${linknacional_public_ajax.empty_folder_text || 'This folder is empty'}</h3>
					<p>${linknacional_public_ajax.empty_folder_desc || 'No files or folders found in this location.'}</p>
				</div>
			`);
			return;
		}
	...
	function createFolderElement(folder, layout) {
		const $element = $(`
			<div class="content-item folder" data-folder-id="${folder.id}" data-folder-name="${folder.name}">
				...
				<div class="content-item-info">${linknacional_public_ajax.folder_text || 'Folder'}</div>
			</div>
		`);
		...
		if (layout === 'list') {
			const $element = $(`
				<div class="content-item file ${file.file_type}" data-file-id="${file.id}" data-file-url="${file.file_url}" data-file-type="${file.file_type}" data-folder-id="${file.folder_id}">
					<i class="${fileIcon}"></i>
					<button class="file-locate-btn" data-folder-id="${file.folder_id}" data-file-name="${file.original_name}" title="${linknacional_public_ajax.open_file || 'Open file'}">
					...
					<button class="file-download-btn" data-file-url="${file.file_url}" data-file-name="${file.original_name}">
						${linknacional_public_ajax.download_text || 'DOWNLOAD'}
					</button>
				</div>
			`);
			return $element;
	...
	function performSearch(searchTerm) {
		...
		isSearchMode = true;
		$('#linknacional-clear-search').show();

		// Clear file selections when performing search
		$treeContainer.find('.file-item-tree').removeClass('selected');
		$container.find('.content-item.file').removeClass('selected-in-tree');

		$content.html('<div class="loading"><i class="fas fa-spinner"></i> ' + (linknacional_public_ajax.searching_text || 'Searching...') + '</div>');

		$.ajax({
			url: linknacional_public_ajax.ajax_url,
			type: 'POST',
			data: {
				action: 'linknacional_frontend_search',
				nonce: nonce,
			...
			success: function (response) {
				if (response.success) {
					...
					$content.html('<div class="error">' + (linknacional_public_ajax.error_search_text || 'Error performing search') + '</div>');
				}
			},
			error: function () {
				$content.html('<div class="error">' + (linknacional_public_ajax.error_search_text || 'Error performing search') + '</div>');
	...
	function renderSearchResults(data, $container) {
		...
		const searchHeader = $(`
			<div class="search-results-header">
				...
				${linknacional_public_ajax.search_results_text || 'Search results for'} "${data.search_term}" - ${linknacional_public_ajax.found_items_text || 'Found'} ${(data.folders.length + data.files.length)} ${linknacional_public_ajax.items_text || 'items'}
			</div>
		`);
		...
		if (folders.length === 0 && files.length === 0) {
			$content.append(`
				<div class="empty-folder">
					<i class="fas fa-search"></i>
					<h3>${linknacional_public_ajax.no_results_text || 'No results found'}</h3>
					<p>${linknacional_public_ajax.no_results_desc || 'Try adjusting your search terms.'}</p>
				</div>
			`);
			return;
		}
	...
	function updateBreadcrumb(breadcrumb, $container) {
		const $breadcrumbContainer = $container.find('#linknacional-current-path');
	...
	function hideSubfoldersRecursively(parentId, $treeContainer) {
		...
		$directChildren.each(function () {
			...
			if ($childToggleBtn.length > 0) {
				...
				$childToggleBtn.attr('title', linknacional_public_ajax.show_subfolders || 'Show Subfolders');
			}
	...
	function loadFolderTree($container) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {
				action: 'linknacional_frontend_get_all_folders',
				nonce: nonce
			},
			success: function (response) {
				if (response.success) {
					...
					$treeContainer.html('<div class="error">' + (linknacional_public_ajax.error_folders_text || 'Error loading folders') + '</div>');
				}
			},
			error: function () {
				$treeContainer.html('<div class="error">' + (linknacional_public_ajax.error_folders_text || 'Error loading folders') + '</div>');
	...
	function renderFolderTree(folders, files, $treeContainer, $mainContainer) {
		...
		$treeContainer.on('click', '.folder-item-content-public', function (e) {
			...
			// Clear file selections when navigating to different folder
			$treeContainer.find('.file-item-tree').removeClass('selected');
			const $container = $treeContainer.closest('.linknacional-filebrowser-public');
			$container.find('.content-item.file').removeClass('selected-in-tree');
		...
		$treeContainer.on('click', '.folder-toggle-btn-public', function (e) {
			...
			if (isExpanded) {
				...
				$btn.attr('title', linknacional_public_ajax.show_subfolders || 'Show Subfolders');
			} else {
				// Show subfolders and files
				$subfolders.addClass('show');
				$subfiles.addClass('show');
				$icon.removeClass('fa-caret-right').addClass('fa-caret-down');
				$btn.attr('title', linknacional_public_ajax.hide_subfolders || 'Hide Subfolders');
			}
		});
		...
		folders.forEach(function (folder) {
			...
			if (hasChildren) {
				folderItemHtml = `
					...
					<button class="folder-toggle-btn-public" data-folder-id="${folder.id}" title="${linknacional_public_ajax.show_subfolders || 'Show Subfolders'}">
